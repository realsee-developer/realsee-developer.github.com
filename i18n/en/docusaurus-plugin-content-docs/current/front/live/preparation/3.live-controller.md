...
```js
live.broadcast(data /* broadcast data*/, ['user_id_1', 'user_id_2']/* userId */)
```
...
- `inite(): void` initialize completed events.
...
```ts
live.$RTC.on('userVolumes', (userVolumes: UserVolumme[]) => {
...
const rtcInstallation = new VRWebViewRTC({
  jsBridge,
  getVoiceSign: () => {},
})
