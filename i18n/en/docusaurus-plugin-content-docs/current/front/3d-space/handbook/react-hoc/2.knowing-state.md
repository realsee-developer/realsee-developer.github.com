...
> **Previous chapter recalls: Show 3D space** <br/> - You know what you are **Work**, and how to get and load.<br/> - How to display three-dimensional spaces and develop components again to control three-dimensional space.
...
**src/2. Nowing-state/useWindowDimensions.tsx**
```tsx
import React, { Component, ComponentClass } from "react";
...
const App = compose(
  withFetchWork(workURL),
  withWindowDimensions()
)(class <ends: Component Work, windowDimensions: { width: number, height: number }}> {
  render() {
    const { work, windowDimensions } = this.props;
    return <FiveProvider initialWork={work}>
      <FiveCanvas width={windowDimensions.width} height={windowDimensions.height}/>
