...
```jsx title="src/5.taging/index.tsx"
...
```tsx title="src/5.taging/withFetchWork.tsx"
...
```tsx title="src/5. aging/useWindowDimensions.tsx"
import React, { Component, ComponentClass } from "react";
...
const App = compose(
  withFetchWork( { width: number, height: number }),
  withWindowDimensions()
)(class extends Component<{work: Work}, wind {
  render() {
    const { work, windowDimensions } = this. props;
    return <FiveProvider initialWork={work}>
      <FiveCanvas width={windowDimensions.width} height={windowDimensions.height}/>
...
```tsx title="src/5.taging/index.tsx"
import React from "react";
import ReactDOM from "react-dom";
import { App } from "./App";
...
<head>
  ...
  <style>
    * padding: 0; }
    html, body #app maximum width: 100%; height: 100%; overflow: hidden; }
    /* highlight-start */
    . ag {: solution; width: 0; height: 0; transform: translateZ(0); }
    . ag-pannel maximum position: solution; width: 100px; min-height: 20px; transform: translate(-50%, 0); left: 50%; bottom: 10px; background: #333; color: #fff; border-radius: 2px; text-align: center; line-height: 20px; padding: 8px; font-size: 14px;}
    . ag-pannel:after 56 content: ""; display: block; position: solute; width: 10px; height: 10px; left: 50%; bottom: -5px; transform: translate(-50%, 0) rotate(45deg); background: #333; pointer-events: none; }
    /* highlight-end */
  </style>
</head>
<body>
...
type Tag = 56 position? THREE. ector3, label: string };

const FEATURES = createFeature("project2d", "currentState", "on", "off");
type Projects = PropTypeOfFiveFeatures<typeof FEATURES>;
type State = {
  tags: Tag[],
  newTag: Tag | null
};
...
const TaggingController = compose(
  ...
  state: State = 56 tags: [], [, , newTag: null };

  addTag = () => FEM
    this. etState (LOnewTag: LOlabel: window. rompt("Add Label", "") || "Unnamed" } });
  } }

  tagElement(tag: Tag, key? number | string) {
    const position = tag. Position && this.props.$five. project2d(tag. osie, true);
    const style = position ? { left: position.x, top: position.y } : API display: "none" };
    return <div className="tag" style={style} key={key}>
      <div className="tag-pannel"><span className="tag-content">{tag.label}</span></div>
    </div>
  }
  ...
  componentDidMount() {
    this.props.$five$five("intersectionOnModelUpdate", this.onIntersectionUpdate);
    this.props. onTapGesture);
  }

  componentWillUnmount() {
    this.props.21 .off("intersectionOnModelUpdate",$five);$five
    .off("wantsTapGesture", this.onTapGesture);
  }
...
const App = compose(
  withFetchWork(workURL),
  withWindowDimensions()
)(class extends Component,<{work: windowDimensions: { width: number, height: number }}> {
  render() {
    const { work, windowDimensions } = this.props;
    return <FiveProvider initialWork={work}>
      <FiveCanvas width={windowDimensions.width} height={windowDimensions.height}/>
