...
```jsx title="src/4.points-in-3d/index.tsx"
...
```tsx title="src/4. oints-in-3d/withWindowDimensions.tsx"
import React, { Component, ComponentClass } from "react";
...
const App = compose(
  withFetchWork(workURL),
  withWindowDimensions()
)(class extends Components<{ { width: number, height: number }: }> {
  render() {
    const { work, windowDimensions } = this.props;
    return <FiveProvider initialWork={work}>
      <FiveCanvas width={windowDimensions.width} height={windowDimensions.height}/>
      ...
      <Stack>
        ...
        {this.state.marks.map(point, index) ==>
          ...
          return <Chip
            ...
            onDelete={() > this.setState({marks: this.state.marks.filter((, index_) => index_!== index)})}
          />
}}
        </Stack>
      </Stack>
...
const FEATURES = createFeature("intersectRaycaster", "on", "off");
...
const MarkController = compose(
  ...
  componentDidMount() {
$five.on(" wantsTapGesture", this.onTapGesture);
  }
  componentWillUnmount() {
    this.props.$five.off("wantsTapGesture", this.onTapGesture);
  }
...
const App = compose(
  withFetchWork(workURL),
  withWindowDimensions()
)(classon extends work: Work, windowDimensions: { width: number, height: number }}> {
  render() {
    const { work, windowDimensions } = this.props;
    return <FiveProvider initialWork={work}>
      <FiveCanvas width={windowDimensions.width} height={windowDimensions.height}/>
