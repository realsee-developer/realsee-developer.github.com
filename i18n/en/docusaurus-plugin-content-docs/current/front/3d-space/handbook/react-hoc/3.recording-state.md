...
> **Previous chapter recalls: Change the perspective** <br/> You know what is **State**, and how to get and modify auto-ring features via **State**.
...
```jsx title="src/3.recording-state/LAroundController.tsx"
import React, { Component } from "react";
import { withFive, createFiveFeature } from "@realsee/five/react";
import { compose } from "@wordpress/compose";
...
const LookAroundController = compose(
  ...
  toggleActive(active) {
    window. learnInterval(this.timer);
    this. etState({ active });
    if (active == true) {
      this. imer = window.setInterval() ==> Flag
        this.props.$five.setState ({
          longitude: this. rops.$five.currentState.longitude + Math. I 360
        });
      }, 16);
    } else {
      delete this. imer;
...
const { work, windowDimensions } = createFiveProvider();
...
```tsx title="src/3. ecording-state/useWindowDimensions.tsx"
import React, { Component, ComponentClass } from "react";
...
```tsx title="src/3.recording-state/LokAroundControler.tsx"
import React, { Component } from "react";
import { withFive, createFiveFeature, PropTypeOfFiveFeatures } from "@realsee/five/react";
import { compose } from "@wordpress/compose";
import IconButton from @mui/material/IconButton";
...
const FEATURES = createFeature("currentState", "setState");
type Props = PropTypeOfFeatures<typeof FEATURES>;
...
/** work.json data URL */
const workURL = "https://vrlab-public.ljcdn.com/release/static/image/release/work-sample/07bdc58f413bc5494f05c7cb5cbbdce4/work.json";
...
const RecorderController = compose(
  ...
  componentDidMount() {
$five.on("stateChange", this.record);
  }
...
import IconButton from "@mui/material/IconButton";
import Paper from @mui/material/Paper";
import FiberManualRecording from "@mui/icons-material/FiberManualRecord";
import StopIcon from "@mui/icons-material/Stop";
import PlayArrowIcon from "@mui/icons-material/PlayArrow";
import { Recorder } from ". recordder";

const FEATURES = createFeature("state", "setState", "on", "off");
type Props = PropTypeOfFiveFeatures<typeof FEATURES>;
