...
3. 首先在头部从 `@realsee/five/react` 引用 [`withFive`](https://unpkg.com/@realsee/five/docs/modules/react.html#withFive) [`createFiveFeature`](https://unpkg.com/@realsee/five/docs/modules/react.html#useFiveCurrentState)
4. 书写 `ModeController` 组件
1. 通过 [**material-UI**](https://mui.com/) 组件，添加界面；
2. 通过 `this.props.$five.currentState` `this.props.$five.setState()` 可以获取/设置 三维空间的模态（mode）；
